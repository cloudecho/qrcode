...
public class ZxingCode {
	...
	* @throws NotFoundException
	*/
	public static Result decode(InputStream input, String charset) throws IOException,
	    NotFoundException {
	...
	* @throws IOException
	*/
	public  static void encode(ZxingEntry zxing, InputStream logoStream) throws WriterException, IOException {
	Hashtable<EncodeHintType, Object> hints = new Hashtable<EncodeHintType, Object>();
	// 设置纠错级别(L 7%~M 15%~Q 25%~H 30%),纠错级别越高存储的信息越少
	hints.put(EncodeHintType.ERROR_CORRECTION,
		zxing.getErrorCorrectionLevel());
	...
	writeToFile(bitMatrix, zxing.getFormat(), new File(zxing.getPath()),
		logoStream, zxing.getLogoCornerRadius());
	...
	*/
	static void writeToFile(BitMatrix bitMatrix, String format, File file,
			    InputStream logoStream, int logoCornerRadius) throws IOException {
	BufferedImage bi = toBufferedImage(bitMatrix);
	if (logoStream != null) {
	    int width_4 = bitMatrix.getWidth() / 4;
	    int width_8 = width_4 / 2;
	    ...
	    int height_8 = height_4 / 2;
			...
			/* 读取logo图片信息 */
	    BufferedImage logoimg = ImageIO.read(logoStream);// 实例化一个Image对象。
